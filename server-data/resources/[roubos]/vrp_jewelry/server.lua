...
-- WEBHOOK
-----------------------------------------------------------------------------------------------------------------------------------------
local webhookjoalheria = "https://discordapp.com/api/webhooks/764151109198807070/VxLpTPEH6Okd48xKmnisGWQhGTnz_CV1UX26pznly71Ov-OGwkE9rBxz7Elr_ltmDS7x"
...
function func.checkJewelry(x,y,z,h,sec,tipo)
	...
	if user_id then
		if #policia < 0 then -- Número mínimo de policiais em serviço
			...
			if vRP.getInventoryItemAmount(user_id,"pendrive") >= 1 and vRP.tryGetInventoryItem(user_id,"pendrive",1) and not roubando then
				...
				SetTimeout(sec*1000,function()
					if firewall >= 20 then
						...
						func.callPolice(x,y,z)
						TriggerClientEvent('iniciandojewelry',source,x,y,z,h,sec,tipo,false)
						TriggerClientEvent("Notify",source,"aviso","O computador é protegido pelo <b>firewall</b>, ele bloqueou sua conexão, tente novamente.")
					end
				end)
			else
	...
	for l,w in pairs(policia) do
		...
		if player then
			async(function()
				...
				TriggerClientEvent('chatMessage',player,"Central",{64,64,255},"O roubo começou na ^1Joalheria^0, dirija-se até o local e intercepte o assaltante.")
	...
	if user_id then
		if timers[id] == 0 or not timers[id] then
			...
			SetTimeout(10000,function()
				...
				vRP.giveInventoryItem(user_id,jewels[math.random(5)].item,math.random(20,40))
			end)
		else
			TriggerClientEvent("Notify",source,"aviso","O balcão está vazio, aguarde <b>"..vRP.format(parseInt(timers[id])).." segundos</b> até que a loja reponha-os.")
		end
	end
end