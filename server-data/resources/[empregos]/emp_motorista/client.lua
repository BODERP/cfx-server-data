...
local isInBus = false
local currentStop = 1
local blipLocation = vector3(450.18, -650.94, 28.47) -- Substitua essas coordenadas pelas coordenadas do local do blip
local busJobBlip = nil

-- Evento para criar ou atualizar o blip quando o jogador entra no servidor
AddEventHandler('onClientMapStart', function()
    CreateOrUpdateBusJobBlip()
end)

-- Função para criar ou atualizar o blip
function CreateOrUpdateBusJobBlip()
    if busJobBlip then
        RemoveBlip(busJobBlip)
    end

    busJobBlip = AddBlipForCoord(blipLocation.x, blipLocation.y, blipLocation.z)
    SetBlipSprite(busJobBlip, 513) -- Defina o sprite do blip como um ícone de ônibus (pode ajustar conforme necessário)
    SetBlipDisplay(busJobBlip, 4)
    SetBlipScale(busJobBlip, 0.8)
    SetBlipColour(busJobBlip, 5) -- Cor azul para o blip de ônibus
    SetBlipAsShortRange(busJobBlip, true)

    BeginTextCommandSetBlipName("STRING")
    AddTextComponentString("Trabalho de Ônibus") -- Nome do blip
    EndTextCommandSetBlipName(busJobBlip)
end

-- Evento para iniciar o trabalho quando o jogador está perto do blip
RegisterCommand('startbusjob', function()
    TriggerEvent('busPickup', currentStop) -- Simular o início do trabalho
end, false)
