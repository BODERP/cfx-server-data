...
function src.checkPayment()
	...
	if user_id then
		if not vRP.hasPermission(user_id,"policia.permissao") then
			...
			local randlist = math.random(100) -- Número aleatório para acionar a policia. 
			if randlist >= 40 and randlist < 90 then -- Ganha dinheiro
			...
			elseif randlist >= 90 then -- Chama a policia
				...
				for k,v in pairs(policia) do
					...
					if player then
						...
						TriggerClientEvent('chatMessage',player,"911",{64,64,255},"Um cidadão foi assaltado e ligou para o COPOM. Verifique o ocorrido!")
						SetTimeout(30000,function() vRPclient.removeBlip(player,blips[ids]) idgens:free(ids) end)
					end
				end
			end
			local randmoney = math.random(100,280)
			if #policia > 0 then
				randmoney = math.random(250,490)
			end
			
			--vRP.giveMoney(user_id,parseInt(randmoney))
			vRP.giveInventoryItem(user_id,"dinheirosujo",parseInt(randmoney))
			TriggerClientEvent("Notify",source,"sucesso","A vítima entregou <b>"..parseInt(randmoney).." reais</b>.",8000)
		end
	end
end
-----------------------------------------------------------------------------------------------------------------------------------------
