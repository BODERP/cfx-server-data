...
-- WEBHOOK
-----------------------------------------------------------------------------------------------------------------------------------------
local webhookequipar = "https://discordapp.com/api/webhooks/744710872915771462/HjnIUQSvLn1yP5KIKR_z68BEzT4vG7fRqeP8aMlVt18T7FH5I1lgzXcW9wPVPMNeK78R"
local webhookenviaritem = "https://discordapp.com/api/webhooks/744710946018295819/g9xfTt7X2dAQJup7N1AWYmK4jBEtV2Ond_8mKiGU_Mw4SxVuexEALB0L0We5dFdndSU7"
local webhookdropar = "https://discordapp.com/api/webhooks/763071042749988884/B8noyYuepmzsfKkpe1y3jTy1hlgiacWKEdm0pMPKb_Jw8liz3JAoLWlyve0iEAp8GYQz"
	...
	if user_id and ramount ~= nil and parseInt(ramount) >= 0 and not actived[user_id] and actived[user_id] == nil then
		if type == "usar" then
				if itemName == "bandagem" then
					if vRPclient.getHealth(source) > 70 and vRPclient.getHealth(source) < 300 then
			...
			elseif itemName == "water" then
				if vRP.tryGetInventoryItem(user_id,"water",1) then
					...
					SetTimeout(20000,function()
						...
						TriggerClientEvent('cancelando',source,false)
						vRPclient._DeletarObjeto(source)
						TriggerClientEvent("Notify",source,"sucesso","Você bebeu toda a garrafa.",8000)
					end)
					SetTimeout(30000,function()
						TriggerClientEvent('energeticos',source,false)
						TriggerClientEvent("Notify",source,"aviso","O efeito do energético passou e o coração voltou a bater normalmente.",8000)
			...
			elseif itemName == "lockpick" then
				...
				if vRP.getInventoryItemAmount(user_id,"lockpick") >= 1 and vRP.tryGetInventoryItem(user_id,"lockpick",1) and vehicle then
					...
					TriggerClientEvent("progress",source,20000,"furtando")
					SetTimeout(20000,function()
						actived[user_id] = nil
						TriggerClientEvent('cancelando',source,false)
						vRPclient._stopAnim(source,false)
			...
			elseif itemName == "masterpick" then
				...
				if vRP.getInventoryItemAmount(user_id,"masterpick") >= 1 and vRP.tryGetInventoryItem(user_id,"masterpick",1) and vehicle then
					...
					TriggerClientEvent('cancelando',source,true)
					vRPclient._playAnim(source,false,{{"amb@prop_human_parking_meter@female@idle_a","idle_a_female"}},true)
					TriggerClientEvent("progress",source,60000,"furtando")
					SetTimeout(60000,function()
						actived[user_id] = nil
						TriggerClientEvent('cancelando',source,false)
						vRPclient._stopAnim(source,false)
				...
				if not vRPclient.isInVehicle(source) then
					...
					if vehicle then
						if vRP.hasPermission(user_id,"mecanico.permissao") then
							...
							TriggerClientEvent('cancelando',source,true)
							vRPclient._playAnim(source,false,{{"mini@repair","fixing_a_player"}},true)
							TriggerClientEvent("progress",source,45000,"reparando veículo")
			...
			elseif itemName == "repairkitdois" then
				if not vRPclient.isInVehicle(source) then
					local vehicle = vRPclient.getNearestVehicle(source,3.5)
					if vehicle then
						if vRP.hasPermission(user_id,"mecanico.permissao") then
							...
							if vRP.tryGetInventoryItem(user_id,"repairkitdois",1) then
								actived[user_id] = true
								TriggerClientEvent('Creative:Update',source,'updateMochila')
								TriggerClientEvent('cancelando',source,true)
								vRPclient._playAnim(source,false,{{"mini@repair","fixing_a_player"}},true)
				...
				if not vRPclient.isInVehicle(source) then
					...
					if vehicle then
						if vRP.hasPermission(user_id,"mecanico.permissao") then
							...
							TriggerClientEvent('cancelando',source,true)
							vRPclient._playAnim(source,false,{{"mini@repair","fixing_a_player"}},true)
							TriggerClientEvent("progress",source,300,"reparando pneus")
