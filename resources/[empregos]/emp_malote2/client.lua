...
Citizen.CreateThread(
	function()
		while true do
			...
			if model == 1747439474 then -- HASH DO VEÍCULO
...
Citizen.CreateThread(
	function()
		...
		while true do
			...
			if
				GetDistanceBetweenCoords(-623.34,-1598.62,26.76, GetEntityCoords(GetPlayerPed(-1))) <
					10.0
			 then
				--DrawMarker(21,-344.98,-1562.29,25.24 - 0.9001,0,0,0,0,0,0,10.0,10.0,1.0,0,95,140,50,0,0,0,0)
				DrawMarker(21,-623.34,-1598.62,26.76 -0.4,0,0,0,0,0,0,1.5,1.5,1.0,255,230,100,150,0,0,0,1) -- local para pegar o serviço de dentro do caminhão
			end
			-- Coordenada de onde inicia a rota -1391.6,-450.95,34.09
			if
				GetDistanceBetweenCoords(-1391.6,-450.95,34.09, GetEntityCoords(GetPlayerPed(-1))) <
					2.0
			 then
				if onjob == false then
					...
					if (IsControlJustReleased(1, 38)) then
						if IsVehicleModel(GetVehiclePedIsIn(GetPlayerPed(-1), true), GetHashKey("stuckade")) then
							TriggerEvent("Notify", "sucesso", "Siga coletando os malotes pela cidade.")
							onjob = true
						else
							Citizen.Wait(10) --Citizen.Wait(1)
							TriggerEvent("Notify", "negado", "Você precisa estar com o carro forte.")
						end
					end
				else
...
function openMenuTrashCar(nearTrashCar)
	if nearTrashCar and CanLeaveBox then
		drawTxt("PRESSIONE ~g~G~s~ PARA ~y~GUARDAR O MALOTE", 1, 1, 0.5, 0.90, 0.5, 255, 255, 255, 200)
		if (IsControlJustPressed(1, 47)) then
			...
			TriggerServerEvent("damn_malote:rewardMalote")
