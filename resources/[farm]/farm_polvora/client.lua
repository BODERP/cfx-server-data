...
emP = Tunnel.getInterface("farm_polvora")
-----------------------------------------------------------------------------------------------------------------------------------------
-- VARIAVEIS
-----------------------------------------------------------------------------------------------------------------------------------------
local blips = false
local servico = false
local selecionado = 0
local CoordenadaX = 98.2
local CoordenadaY = 6619.68
local CoordenadaZ = 32.44
local processo = false
local segundos = 0
--98.2,6619.68,32.44 -- atual
-- 457.52, -1498.25, 28.18 -- anterior
-----------------------------------------------------------------------------------------------------------------------------------------
-- RESIDENCIAS
-----------------------------------------------------------------------------------------------------------------------------------------
local locs = {
			...
			if distance <= 3 then
				...
				if distance <= 1.5 then
					...
					if IsControlJustPressed(0,38) --[[and emP.checkPermission()]] then
						...
						TriggerEvent("Notify","sucesso","Vá Coletar a Pólvora.")
					end
				end
			end
		end
		...
		if servico then
			...
			if distance <= 3 then
				...
				if distance <= 1.2 then
					drawTxt("PRESSIONE  ~r~E~w~  PARA ~y~COLETAR A PÓLVORA",4,0.5,0.90,0.50,255,255,255,200)
